...
export default function LostReportAdminTable() {
  const {
    ...
    mutate,
    isValidating,
  }: SWRResponse<LostReportAdmin[], Error> = useSWR("/lost", fetcher);
    ...
    <>
                          ...
                          {report.status === "PENDING" && (
                            <>
                              <Button
                                ...
                                onClick={() =>
                                  setAlertAction({ id: report.id, status: "APPROVED" })
                                }
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  setAlertAction({ id: report.id, status: "REJECTED" })
                                }
                                disabled={loadingAction === report.id}
                              >
                                {loadingAction === report.id ? (
          ...
          {selectedReport && (
            <div className="space-y-6">
              ...
              <div className="grid gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Lokasi Hilang</label>
                ...
                <div className="border-t pt-4">
                  <label className="text-sm font-medium text-muted-foreground">Informasi Pelapor</label>
                  <div className="mt-2 space-y-2">
                    <div className="flex items-center gap-2">
                      <User className="h-4 w-4 text-muted-foreground" />
                      <span className="text-sm">{selectedReport.user.name}</span>
                    </div>

                    <div className="flex items-center gap-2">
                      <Mail className="h-4 w-4 text-muted-foreground" />
                      <span className="text-sm">{selectedReport.user.email}</span>
                    </div>

                    {selectedReport.user.notelp && (
                      <div className="flex items-center gap-2">
                        <Phone className="h-4 w-4 text-muted-foreground" />
                        <span className="text-sm">{selectedReport.user.notelp}</span>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <AlertDialog open={!!alertAction} onOpenChange={() => setAlertAction(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {alertAction?.status === "APPROVED" ? "Setujui Laporan?" : "Tolak Laporan?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => alertAction && handleStatus(alertAction.id, alertAction.status)}
              className={alertAction?.status === "REJECTED" ? "bg-red-600 hover:bg-red-700" : ""}
            >
