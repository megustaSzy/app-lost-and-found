...
const lostMeFetcher = (url: string) =>
  api.get(url).then((res) => res.data.data);

const foundFetcher = (url: string) =>
  api.get(url).then((res) => res.data.data);

export default function UserDashboard() {
  const { user, loading: userLoading } = useUser();

  const { data: myLostReports } = useSWR(
    user ? "/lost/me" : null,
    lostMeFetcher
  );

  const { data: foundReports } = useSWR(
    user ? "/found" : null,
    foundFetcher
  );
    ...
    return (
      <div className="flex min-h-screen items-center justify-center p-4">
        <Alert variant="destructive" className="max-w-md">
          ...
          <AlertDescription>
            Anda harus login untuk mengakses dashboard ini!
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  const chartData = [
    {
      name: "Hilang Saya",
      jumlah: myLostReports.length,
      fill: "#f97316",
    },
    {
      name: "Ditemukan",
      jumlah: foundReports.length,
      fill: "#10b981",
    },
  ];
                ...
                >
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between">
                      ...
                      <div
                        className={`p-3 rounded-xl bg-white shadow-md ${stat.iconColor}`}
                      >
                    ...
                    <div
                      className={`mt-4 h-1.5 rounded-full bg-gradient-to-r ${stat.gradient}`}
                    ></div>
                  </CardContent>
                </Card>
              );
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-4 bg-white/10 rounded-xl backdrop-blur">
                    ...
                    <span className="text-2xl font-bold">
                      {totalReports > 0
                        ? Math.round(
                            (foundReports.length / totalReports) * 100
                          )
                        : 0}
                      %
                    </span>
